...
class PGAgent(base_agent.BaseAgent):
    ...
    def _calc_return(self, r, done):
        ...
        return_t[-1] = r[-1]
        for i in range(len(return_t)-2, -1, -1):
            if done[i]:
                return_t[i] = r[i]
            else:
                return_t[i] = r[i] + self._discount * return_t[i+1] # .clone() to sovle the problem of in-place operation
        return return_t
    ...
    def _calc_adv(self, norm_obs, ret):
        ...
        # placeholder
        value = self._model.eval_critic(norm_obs)[:,0]
        adv = ret - value
        return adv
    ...
    def _calc_critic_loss(self, norm_obs, tar_val):
        ...
        # placeholder
        y = self._model.eval_critic(norm_obs)[:,0]
        loss = ((tar_val - y)**2).mean( axis=-1 )
        return loss
    ...
    def _calc_actor_loss(self, norm_obs, norm_a, adv):
        ...
        # placeholder
        logp = - self._model.eval_actor(norm_obs).log_prob(norm_a)
